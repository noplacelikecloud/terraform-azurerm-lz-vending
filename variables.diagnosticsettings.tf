...
variable "diagnostic_settings" {
  ...
  validation {
    condition     = (!var.vnet_diagnostic_settings_enabled && !var.rsv_diagnostic_settings_enabled) || (var.diagnostic_settings.deploy_log_analytics_workspace == false ? (var.diagnostic_settings.existing_log_analytics_workspace_id != "" || var.diagnostic_settings.storage_type == "StorageAccount" && var.diagnostic_settings.existing_storage_account_id != "") : true)
  ...
  validation {
    condition     = (!var.vnet_diagnostic_settings_enabled && !var.rsv_diagnostic_settings_enabled) || (var.diagnostic_settings.deploy_log_analytics_workspace == true ? (var.diagnostic_settings.log_analytics_workspace_name != "") : true)
  ...
  validation {
    condition     = (!var.vnet_diagnostic_settings_enabled && !var.rsv_diagnostic_settings_enabled) || (contains(["LogAnalytics", "StorageAccount"], var.diagnostic_settings.storage_type))
    error_message = "The storage_type must be either 'LogAnalytics' or 'StorageAccount'."
  }

  validation {
    condition     = (!var.vnet_diagnostic_settings_enabled && !var.rsv_diagnostic_settings_enabled) || (var.diagnostic_settings.storage_type == "StorageAccount" ? (var.diagnostic_settings.retention_days == 0) : true)
    error_message = "If you are using a Storage Account for diagnostics, retention_days must be set to 0."
  }

  validation {
    condition     = (!var.vnet_diagnostic_settings_enabled && !var.rsv_diagnostic_settings_enabled) || (var.diagnostic_settings.storage_type == "LogAnalytics" ? (var.diagnostic_settings.retention_days > 0) : true)
    error_message = "If you are using Log Analytics for diagnostics, retention_days must be greater than 0."
  }

  validation {
    condition     = (!var.vnet_diagnostic_settings_enabled && !var.rsv_diagnostic_settings_enabled) || (var.diagnostic_settings.deploy_log_analytics_workspace ? (var.diagnostic_settings.resource_group_key_reference != null || var.diagnostic_settings.resource_group_name_existing != null) : true)
